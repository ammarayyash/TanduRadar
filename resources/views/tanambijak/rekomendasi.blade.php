...
<section class="section" style="padding-top:100px;">
    <div class="container">
        ...
        <div class="filter-bar">
            ...
            <form method="GET" action="{{ route('rekomendasi') }}" style="display:flex;gap:10px;align-items:center">
        ...
        <!-- Status Peringatan -->
        @if($komoditasMerah->isNotEmpty())
        <div class="card" style="border-color:var(--status-merah); margin-bottom:30px;">
            <h3 style="color:var(--status-merah); margin-bottom: 10px;">⛔ Hindari Musim Ini di {{ $kecamatan }}</h3>
            <div style="display:flex; flex-wrap:wrap; gap:8px; margin-bottom:14px;">
                @foreach($komoditasMerah as $k)
                    <span class="badge badge-merah" style="font-size:.9rem; padding:6px 14px;">{{ $k }}</span>
                @endforeach
            </div>
            <p style="font-size:.88rem; color:var(--text-muted); margin:0;">
                "Hindari {{ $komoditasMerah->first() }} musim ini! Lahan jenuh — risiko anjlok harga sangat tinggi. Pertimbangkan alternatif di bawah."
            </p>
        </div>
        @endif

        @if($komoditasHijau->isNotEmpty())
        <div class="card" style="border-color:var(--status-hijau); margin-bottom:30px;">
            ...
            <div style="display:flex; flex-wrap:wrap; gap:8px;">
                @foreach($komoditasHijau as $k)
                    <span class="badge badge-hijau" style="font-size:.9rem; padding:6px 14px;">{{ $k }}</span>
                @endforeach
            </div>
        </div>
        @endif
            ...
            <div class="card card-alternatif" style="align-items:flex-start; gap:16px; padding:24px;">
                ...
                <div class="alt-body" style="flex:1">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                        <h4 style="margin:0; font-size:1.1rem;">{{ $alt['nama'] }}</h4>
                        <span class="badge badge-hijau" style="font-size:.8rem;">{{ $alt['keuntungan'] }} profit</span>
                    </div>
                    <div style="display:flex; gap:16px; font-size:.82rem; color:var(--text-muted); margin-bottom:10px;">
                        <span><i class="fas fa-clock"></i> Panen ~{{ $alt['masa_panen'] }} hari</span>
                    </div>
                    <p style="font-size:.85rem; color:var(--text-muted); margin:0 0 12px;">{{ $alt['tips'] }}</p>
                    <a href="{{ route('lapor-tanam') }}?komoditas={{ urlencode($alt['nama']) }}" class="project-link" style="font-size:.85rem;">
        ...
        <div class="card" style="max-width:600px; margin:0 auto;">
            <div class="form-group">
                ...
                <select id="kalkKomoditas" style="width:100%; padding:12px 16px; background:var(--bg-dark); border:1px solid var(--border); border-radius:8px; color:var(--text-main); font-family:inherit;">
                    @foreach($alternatif as $alt)
                        <option value="{{ $alt['masa_panen'] }}" data-nama="{{ $alt['nama'] }}" data-pct="{{ ltrim($alt['keuntungan'], '+') }}">{{ $alt['nama'] }} — panen {{ $alt['masa_panen'] }} hari</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
